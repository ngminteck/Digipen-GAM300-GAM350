...
const int AppConsole::MAXLOG = 10000;
...
int AppConsole::Stricmp(const char* str1, const char* str2)
{
  int d;
...
int AppConsole::Strnicmp(const char* str1, const char* str2, int n)
{
  int d = 0;
...
char* AppConsole::Strdup(const char* str)
{
...
void AppConsole::AddLog(const char* fmt, ...)
{
  ...
  va_end(args);
  std::string tmp = std::to_string(numLogs) + ". " + buf;
  //Items.push_back(Strdup(buf));
  Items.push_back(Strdup(tmp.data()));
  ScrollToBottom = true;
...
void AppConsole::Draw()
{
  ...
  //static const float height = 100.0f;
  //static const float offset = 70.0f;
  ImGui::SetNextWindowPos(ImVec2(0.0f, (float)ASC_SYSTEM.winHeight - height - offset));
  ImGui::SetNextWindowSize(ImVec2((float)ASC_SYSTEM.winWidth, height));
  ...
  for (int i = 0; i < Items.Size; i++)
  {
    ...
    else if (strstr(item, "[Warning]")) col = ImColor(1.0f, 1.0f, 0.0f, 1.0f);
  ...
  case ImGuiInputTextFlags_CallbackHistory:
    {
      ...
      if (prev_history_pos != HistoryPos)
      {
        data->CursorPos = data->SelectionStart = data->SelectionEnd = data->BufTextLen =
          static_cast<int>(snprintf(data->Buf, static_cast<size_t>(data->BufSize), "%s", (HistoryPos >= 0) ? History[HistoryPos] : ""));
        data->BufDirty = true;
      }
    }
  default: break;
