...
/*	error reporting	*/
char result_string_pointer[1024] = "SOIL initialized";
...
)
{
  ...
  if (NULL == img)
  {
    ...
    strcpy_s(result_string_pointer, stbi_failure_reason());
    return 0;
  }
...
)
{
  ...
  {
    strcpy_s(result_string_pointer, "Invalid fake HDR format specified");
    return 0;
  }
...
)
{
  ...
  if ((x_pos_file == NULL) ||
    (x_neg_file == NULL) ||
      ...
      (z_neg_file == NULL))
  {
    strcpy_s(result_string_pointer, "Invalid cube map files list");
    return 0;
  }
  /*	capability checking	*/
  if (query_cubemap_capability() != SOIL_CAPABILITY_PRESENT)
  {
    strcpy_s(result_string_pointer, "No cube map capability present");
    return 0;
  }
...
)
{
  ...
  if ((x_pos_buffer == NULL) ||
    (x_neg_buffer == NULL) ||
      ...
      (z_neg_buffer == NULL))
  {
    strcpy_s(result_string_pointer, "Invalid cube map buffers list");
    return 0;
  }
  /*	capability checking	*/
  if (query_cubemap_capability() != SOIL_CAPABILITY_PRESENT)
...
)
{
  ...
  if (filename == NULL)
  {
    strcpy_s(result_string_pointer, "Invalid single cube map file name");
    return 0;
  }
  /*	does the user want direct uploading of the image as a DDS file?	*/
  if (flags & SOIL_FLAG_DDS_LOAD_DIRECT)
  ...
  for (i = 0; i < 6; ++i)
  {
    if ((face_order[i] != 'N') &&
      (face_order[i] != 'S') &&
        ...
        (face_order[i] != 'D'))
    {
      strcpy_s(result_string_pointer, "Invalid single cube map face order");
      return 0;
    };
  }
  /*	capability checking	*/
  ...
  {
    SOIL_free_image_data(img);
    strcpy_s(result_string_pointer, "Single cubemap image must have a 6:1 ratio");
    return 0;
  }
...
)
{
  ...
  if (buffer == NULL)
  {
    strcpy_s(result_string_pointer, "Invalid single cube map buffer");
    return 0;
  }
  /*	does the user want direct uploading of the image as a DDS file?	*/
  if (flags & SOIL_FLAG_DDS_LOAD_DIRECT)
...
)
{
  ...
  if (data == NULL)
  {
    strcpy_s(result_string_pointer, "Invalid single cube map image data");
    return 0;
  }
  /*	face order checking	*/
  for (i = 0; i < 6; ++i)
...
)
{
  ...
  if (flags & SOIL_FLAG_TEXTURE_RECTANGLE)
  {
    ...
    else
    {
      ...
      strcpy_s(result_string_pointer, "Texture Rectangle extension unsupported");
      return 0;
    }
  }
  ...
  if (tex_id)
  {
    ...
    /*	done	*/
    strcpy_s(result_string_pointer, "Image loaded as an OpenGL texture");
  }
  else
  {
    /*	failed	*/
    strcpy_s(result_string_pointer, "Failed to generate an OpenGL texture name; missing OpenGL context?");
  }
  SOIL_free_image_data(img);
  return tex_id;
}
...
)
{
  ...
  if ((width < 1) || (height < 1))
  {
    strcpy_s(result_string_pointer, "Invalid screenshot dimensions");
    return 0;
  }
  if ((x < 0) || (y < 0))
  {
    strcpy_s(result_string_pointer, "Invalid screenshot location");
    return 0;
  }
  if (filename == NULL)
  {
    strcpy_s(result_string_pointer, "Invalid screenshot filename");
    return 0;
  }
...
)
{
  ...
  {
    strcpy_s(result_string_pointer, "Image loaded");
  }
  return result;
}
...
)
{
  ...
  {
    strcpy_s(result_string_pointer, "Image loaded from memory");
  }
  return result;
}
...
)
{
  ...
  if (save_result == 0)
  {
    strcpy_s(result_string_pointer, "Saving the image failed");
  }
  else
  {
    strcpy_s(result_string_pointer, "Image saved");
  }
  return save_result;
}
  ...
  int loading_as_cubemap)
{
  ...
  {
    /*	we can't do it!	*/
    strcpy_s(result_string_pointer, "NULL buffer");
    return 0;
  }
  if (buffer_length < sizeof(DDS_header))
  {
    /*	we can't do it!	*/
    strcpy_s(result_string_pointer, "DDS file was too small to contain the DDS header");
    return 0;
  }
  ...
  strcpy_s(result_string_pointer, "Failed to read a known DDS header");
  ...
  strcpy_s(result_string_pointer, "DDS header loaded and validated");
  ...
  else
  {
    ...
    {
      /*	we can't do it!	*/
      strcpy_s(result_string_pointer, "Direct upload of S3TC images not supported by the OpenGL driver");
      return 0;
    }
  ...
  if (cubemap)
  {
    ...
    {
      /*	we can't do it!	*/
      strcpy_s(result_string_pointer, "DDS image was a cubemap");
      return 0;
    }
    ...
    {
      /*	we can't do it!	*/
      strcpy_s(result_string_pointer, "Direct upload of cubemap images not supported by the OpenGL driver");
      return 0;
    }
  ...
  else
  {
    ...
    {
      /*	we can't do it!	*/
      strcpy_s(result_string_pointer, "DDS image was not a cubemap");
      return 0;
    }
  ...
  {
    if (buffer_index + DDS_full_size <= static_cast<unsigned int>(buffer_length))
    {
      ...
      /*	it worked!	*/
      strcpy_s(result_string_pointer, "DDS file loaded");
    }
    else
    {
      ...
      strcpy_s(result_string_pointer, "DDS file was too small for expected image data");
    }
  ...
  int loading_as_cubemap)
{
  ...
  if (NULL == filename)
  {
    strcpy_s(result_string_pointer, "NULL filename");
    return 0;
  }
  f = fopen(filename, "rb");
  if (NULL == f)
  {
    ...
    strcpy_s(result_string_pointer, "Can not find DDS file");
    return 0;
  }
  fseek(f, 0, SEEK_END);
  ...
  if (NULL == buffer)
  {
    strcpy_s(result_string_pointer, "malloc failed");
    fclose(f);
    return 0;
  }
  ...
  tex_ID = SOIL_direct_load_DDS_from_memory(
    (const unsigned char *const)buffer, static_cast<int>(buffer_length),
